...
def get_config(_):
    ...
    late_prey_factors = distribs.Product(
        [distribs.Continuous('x', 0.1, 0.8),
         distribs.Continuous('x_vel', -0.01, 0.01),
         distribs.Continuous('y', 1.3, 1.8)
         ],
        y_vel=-0.01, shape='circle', scale=0.07, c0=0.2, c1=1., c2=1.,
    )
    ...
    def state_initializer():
        agent = sprite.Sprite(
            ...
            c1=1., c2=0.66)
        # Occluder
        min_occluder_rad = 0.3
        occluder_rad = np.random.rand() * (0.45 - (min_occluder_rad))  + min_occluder_rad
        occluder_rad = 0.35
        ...
        opacity = np.random.choice([200, 255], p=[0.1, 0.9])
